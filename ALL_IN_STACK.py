...
f = h5py.File(FILE_NAME + '_f.h5', 'r')


log_file = open(FILE_NAME + "_log.txt", "a")
...
f2= h5py.File(FILE_NAME + '_cwt.h5', 'r')
data_swd  = f2["CWT_CH_0"][:,:]
...
for i in range(N_CHAN):
    curve = HDF5Plot()
    curve.setHDF5(f[FILE_NAME][:,i]-2*i)
...
for i in range(N_CHAN):
    curve = HDF5Plot()
    curve.setHDF5(f[FILE_NAME][:,i]-2*i)  #,  pen=d_col[i+1], name=d_chan[i+1])
    curve.setPen(d_col[i+1])
...
img = pg.ImageItem(data_swd[1000:2000,:])
...
def updatePlot():
    ...
    img.setImage(data_swd[np.int(region.getRegion()[0]):np.int(region.getRegion()[1]),:])
def updateRegion():
